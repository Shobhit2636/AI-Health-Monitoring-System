...
import { Brain, Activity, Heart, TrendingUp, AlertCircle, CheckCircle, Loader2 } from "lucide-react";
import toast from "react-hot-toast";
import { predictionsAPI } from "../services/api";

const RISK_CONFIG: Record<string, { color: string; bg: string; bar: string; label: string }> = {
  low:      { color: "text-green-700 dark:text-green-400",  bg: "bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800",  bar: "bg-green-500",  label: "Low Risk" },
  moderate: { color: "text-yellow-700 dark:text-yellow-400", bg: "bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-800", bar: "bg-yellow-500", label: "Moderate Risk" },
  high:     { color: "text-orange-700 dark:text-orange-400", bg: "bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800", bar: "bg-orange-500", label: "High Risk" },
  critical: { color: "text-red-700 dark:text-red-400",       bg: "bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800",       bar: "bg-red-500",    label: "Critical Risk" },
};
    ...
    <div className={`rounded-xl border p-5 ${cfg.bg}`}>
      <div className="flex items-center gap-3 mb-4">
        {result.risk_level === "low"
          ? <CheckCircle size={22} className={cfg.color} />
          : <AlertCircle size={22} className={cfg.color} />}
        <div>
          <p className={`font-semibold ${cfg.color}`}>{cfg.label}</p>
          <p className="text-xs text-gray-500 dark:text-gray-400 capitalize">{result.prediction_type?.replace(/_/g," ")} Prediction</p>
        </div>
      ...
      <div>
        <p className="text-xs font-semibold text-gray-600 dark:text-gray-400 mb-2">Recommendations:</p>
      ...
      <p className="text-xs text-gray-400 dark:text-gray-500 mt-3">Confidence: {Math.round((result.confidence||0.85)*100)}%</p>
    </div>
  );
}
...
export default function Predictions() {
  ...
  const [diabetesForm, setDiabetesForm] = useState({ glucose:120, bmi:25, age:35, blood_pressure:80, pregnancies:0, insulin:80, diabetes_pedigree:0.5 });
  const [heartForm, setHeartForm]       = useState({ age:50, sex:1, chest_pain_type:0, resting_bp:120, cholesterol:200, max_heart_rate:150, exercise_angina:0, st_depression:0.0 });
  const [generalForm, setGeneralForm]   = useState({ age:35, blood_pressure_systolic:120, blood_pressure_diastolic:80, blood_glucose:90, bmi:22 });
  ...
  const tabs = [
    { id:"diabetes" as TabType, label:"Diabetes",      icon:Activity },
    { id:"heart"    as TabType, label:"Heart Disease",  icon:Heart },
    { id:"general"  as TabType, label:"General Health", icon:TrendingUp },
  ];
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Enter your health data for AI-powered risk assessment</p>
      </div>
        ...
        {tabs.map(({ id, label, icon: Icon }) => (
          <button key={id} onClick={() => { setActiveTab(id); setResult(null); }}
            className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-sm font-medium rounded-lg transition-all ${
              activeTab===id ? "bg-white dark:bg-gray-600 shadow text-blue-600 dark:text-blue-400" : "text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200"
            }`}>
            <Icon size={15} />{label}
          </button>
        ))}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
        ...
        {activeTab==="heart" && (
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Chest Pain Type</label>
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Exercise Angina</label>
        ...
        <button onClick={handleSubmit} disabled={loading}
          ...
          {loading ? <><Loader2 size={16} className="animate-spin"/> Analyzing...</> : <><Brain size={16}/> Run AI Prediction</>}
        </button>
      </div>
      ...
      <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4 text-xs text-amber-700 dark:text-amber-400">
        <strong>Disclaimer:</strong> These are AI-generated predictions for informational purposes only. Always consult a qualified healthcare professional.
      </div>
    </div>
  );
}
